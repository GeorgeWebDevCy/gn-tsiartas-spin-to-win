...
Version 2.3.21 reissues the proven 2.3.19 codebase so the wheel once again favours "try again" outcomes while respecting the
...
### 2.3.21
- Reissues the 2.3.19 baseline to roll back the spin distribution adjustments introduced in 2.3.20.
- Bumps plugin metadata and documentation references to version 2.3.21 for release packaging.
...
- Bumps plugin metadata and documentation references to version 2.3.19 for release packaging.

### 2.3.18
- Paces voucher awards to honour the configured Friday quotas throughout the promotion window.
- Bumps plugin metadata and documentation references to version 2.3.18 for release packaging.

### 2.3.17
- Displays a reminder under the wheel date that Spin to Win applies to purchases above €50.
- Bumps plugin metadata and documentation references to version 2.3.17 for release packaging.

### 2.3.16
- Hard-codes the wheel hub logo to tsiartassupermarket.com for consistent branding across every spin.
- Bumps plugin metadata and documentation references to version 2.3.16 for release packaging.

### 2.3.15
- Copies the wheel logo into the WordPress uploads directory whenever it is missing so the front-end always references `wp-content`.
- Bumps plugin metadata and documentation references to version 2.3.15 for release packaging.

### 2.3.14
- Mirrors the plugin's public image assets into the WordPress uploads directory so logos and wheel artwork stay accessible for logged-out visitors.
- Bumps plugin metadata and documentation references to version 2.3.14 for this accessibility update.

### 2.3.13
- Verifies the wheel experience and updater flow against current WordPress maintenance builds to keep deployments worry-free.
- Bumps plugin metadata and documentation references to version 2.3.13 for release packaging.
...
- Bumps plugin metadata and documentation references to version 2.3.10 for release packaging.

### 2.3.9
- Adds console logging that reports the resolved prize, slice index, and computed rotation to assist with real-time debugging.
- Bumps plugin metadata and documentation references to version 2.3.9 for release packaging.

### 2.3.8
- Synchronises the plugin header, version constant, and public readme files to 2.3.8.
- Confirms the GitHub-powered updater remains pointed at the `main` branch for distributing the 2.3.8 build.
